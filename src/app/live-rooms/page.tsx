...
import { Radio, Plus, Search, Video, Mic, X, Check, Database, RefreshCw, Sparkles, Users } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { supabase } from '@/utils/supabase/client';

const LOBBY_CHANNEL_NAME = 'masar_global_live_lobby_v2';
const STORAGE_KEY = 'masar_active_rooms_cache_v2';
...
export default function LiveRoomsLobbyPage() {
  ...
  const [subjectsList, setSubjectsList] = useState<string[]>([
    'الكل', 'الفيزياء', 'الكيمياء', 'الرياضيات', 'اللغة العربية', 'اللغة الإنكليزية', 'العلوم العامة'
  ]);
  ...
  // 1. Helper to sync & deduplicate rooms list
  function mergeRooms(newRooms: LiveRoom[]) {
    setRooms((prev) => {
      const roomMap = new Map<string, LiveRoom>();
      // Preserve existing
      prev.forEach((r) => roomMap.set(r.id, r));
      // Overwrite with new/updated
  ...
  // 2. Load cached rooms on initial mount
  useEffect(() => {
    try {
  ...
  // 4. Supabase Realtime Channel Engine (Presence & Broadcast)
  useEffect(() => {
    const channel = supabase.channel(LOBBY_CHANNEL_NAME, {
      config: {
        presence: { key: profile?.full_name || `user-${Math.random().toString(36).substring(7)}` },
        broadcast: { self: true },
      },
    });
    ...
    // A. Listen for Broadcast Events when any room is created or updated
    channel.on('broadcast', { event: 'ROOM_CREATED' }, (payload) => {
      if (payload?.payload?.room) {
        mergeRooms([payload.payload.room]);
      }
    });

    // B. Listen for Presence State to discover rooms hosted by active online users
    channel.on('presence', { event: 'sync' }, () => {
      const presenceState = channel.presenceState();
      const discoveredRooms: LiveRoom[] = [];
      ...
      Object.keys(presenceState).forEach((key) => {
        ...
        if (presences && presences.length > 0) {
          presences.forEach((p) => {
            if (p.hosted_room) {
              discoveredRooms.push(p.hosted_room);
            }
          });
        }
      });
  ...
  async function handleCreateRoom(e: React.FormEvent) {
    ...
    const newRoomObj: LiveRoom = {
      ...
      description: newDescription.trim() || 'غرفة بث ومذاكرة تفاعلية حية.',
      createdAt: 'الآن',
    };
    ...
    // Broadcast over Supabase Realtime channel to all online users
    try {
      if (channelRef.current) {
        ...
        // Track presence with hosted_room
        channelRef.current.track({
          user: profile?.full_name || 'طالب مسار',
          hosted_room: newRoomObj,
        });
      }
    } catch (err) {
      console.warn('Realtime broadcast room creation:', err);
    }

    // Try optional Supabase DB insert (gracefully handles if table exists)
    try {
      ...
      console.warn('DB table insert (optional fallback):', err);
    }

    mergeRooms([newRoomObj]);
        ...
        <header className="rounded-3xl border-2 border-[#282825] bg-[#ffd64d] p-6 sm:p-8 shadow-[6px_6px_0_#282825] flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-black border-2 border-[#282825] ${isConnected ? 'bg-[#cce6b4] text-[#15803d]' : 'bg-white text-[#282825]'}`}>
                <Database className="w-3.5 h-3.5 text-[#ff5636]" />
                <span>{isConnected ? 'متصل بـ Supabase Realtime ⚡' : 'جاري الاتصال...'}</span>
              </span>
            </div>
            ...
            <p className="text-xs sm:text-sm font-bold text-[#282825]/80 max-w-2xl leading-relaxed">
              تفاعل مباشرة مع زملائك والمدرسين صوتياً ومرئياً، وتظهر جميع الغرف فور إنشائها لكافة الطلاب في نفس اللحظة عبر Supabase.
            </p>
          </div>
            ...
            <div className="flex items-center justify-between border-b-2 border-[#282825]/10 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <small className="text-[11px] font-bold text-[#5f5f59]">مزامنة فورية حية عبر Supabase Realtime</small>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleCreateRoom} className="space-y-4 text-xs font-bold">
              ...
              <div className="pt-2 flex justify-end gap-2">
                ...
                >
                  ...
                  <span>{isSubmitting ? 'جاري البث...' : 'انطلق الآن 🚀'}</span>
                </button>
              </div>
            </form>
