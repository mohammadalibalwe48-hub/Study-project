...
import { useStudyRoomCall, type CallDebugInfo, type RemoteRoomStream, type RoomParticipant } from './useStudyRoomCall';
  ...
  return (
    <div className="relative min-h-56 overflow-hidden rounded-2xl border-2 border-[#282825] bg-[#282825] shadow-[4px_4px_0_#d8bcff]">
      ...
      <video
        ...
        autoPlay
        playsInline
        className={`h-full min-h-56 w-full object-cover ${hasVideo ? 'block' : 'hidden'}`}
      />
      {!hasVideo && (
        <div className="flex min-h-56 flex-col items-center justify-center p-4 text-center text-white">
          <div className="relative flex items-center justify-center">
            <UserRound className="h-14 w-14 text-[#bce9fa]" />
            {micEnabled && (
              <span className="absolute -top-1 -right-1 flex h-4 w-4">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#5fae44] opacity-75"></span>
                <span className="relative inline-flex h-4 w-4 rounded-full bg-[#5fae44]"></span>
              </span>
            )}
          </div>
          <p className="mt-3 text-lg font-black">{name}</p>
          <div className="mt-2 flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1 text-xs font-bold">
            {micEnabled ? <Mic className="h-3.5 w-3.5 text-[#5fae44]" /> : <MicOff className="h-3.5 w-3.5 text-[#ff5636]" />}
            <span>{micEnabled ? 'الميكروفون يعمل' : 'مكتوم'}</span>
          </div>
        </div>
      )}
...
function ActiveRoom({ room, userId, profileName, members, remoteStreams, localStream, chatMessages, messageInput, setMessageInput, sendMessage, messageError, chatEndRef, leaveRoom, timerDuration, timerSecondsLeft, timerActive, setTimerActive, selectTimer, formatTime, soundMode, soundVolume, setSoundVolume, setSoundMode, micEnabled, cameraEnabled, toggleMic, toggleCamera, localVideoRef, mediaError, debugInfo }: { room: StudyRoom; userId: string; profileName: string; members: RoomParticipant[]; remoteStreams: RemoteRoomStream[]; localStream: MediaStream | null; chatMessages: RoomMessage[]; messageInput: string; setMessageInput: (value: string) => void; sendMessage: (event: FormEvent) => void; messageError: string; chatEndRef: React.RefObject<HTMLDivElement | null>; leaveRoom: () => void; timerDuration: number; timerSecondsLeft: number; timerActive: boolean; setTimerActive: (value: boolean) => void; selectTimer: (duration: number) => void; formatTime: (seconds: number) => string; soundMode: 'off' | 'rain' | 'binaural' | 'hum'; soundVolume: number; setSoundVolume: (value: number) => void; setSoundMode: (value: 'off' | 'rain' | 'binaural' | 'hum') => void; micEnabled: boolean; cameraEnabled: boolean; toggleMic: () => void; toggleCamera: () => void; localVideoRef: React.RefObject<HTMLVideoElement | null>; mediaError: string; debugInfo: CallDebugInfo }) {
  ...
  const statusColor = debugInfo.connectionState === 'connected' ? 'bg-[#5fae44]' : debugInfo.connectionState === 'new' || debugInfo.connectionState === 'connecting' ? 'bg-[#ffd64d]' : 'bg-[#ff5636]';
